...
def handle_output_size(size_name, asset_file, max_width)
  ...
  output_dims_path = output_path.sub("_output#{output_ext}", "_#{output_w}_#{output_h}#{output_ext}")
  output_dims_url = output_url.sub("_output#{output_ext}", "_#{output_w}_#{output_h}#{output_ext}")
  unless File.symlink?(output_dims_path)
    File.symlink("#{basename}_#{size_name}#{output_ext}", output_dims_path)
  end
...
Dir["assets/*.json"].each do |asset_file|
  ...
  thumb_props = handle_output_size("thumbnail", asset["file"], 100)
